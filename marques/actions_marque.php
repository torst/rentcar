...
<?php
      ...
      switch ( $option ) {
		  ...
		  case 'insert':
		  	if(isset($_SESSION["ajouter_marque_vehicule"]))
		    	insert();
		    break;
		
		  case 'delete':
		  	if(isset($_SESSION["supprimer_marque_vehicule"]))
		    	delete();
		    break;
...
if(isset($_POST['id_marque'])){
	...
	$erreur = 0;
	if(strcmp($nom,"")==0) {
		$erreur_nom = "Veuillez renseigner le nom de la marque";
		$erreur=1;
	}
	...
	if(!empty($_FILES['marque']['tmp_name']) AND is_uploaded_file($_FILES['marque']['tmp_name'])){
		...
		if(filesize($_FILES['marque']['tmp_name'])<12000000){
			//On vérifie maintenant le type de l'image à l'aide de la fonction getimagesize()
			list($largeur, $hauteur, $type, $attr)=getimagesize($_FILES['marque']['tmp_name']);
			...
			if(($type==IMAGETYPE_JPEG)||($type==IMAGETYPE_PNG)||($type==IMAGETYPE_GIF)){
				...
				if($type==IMAGETYPE_JPEG) {
					$path = "../img/marques/marque".$valeur.".jpg";
					$path_sql = "img/marques/marque".$valeur.".jpg";
					$path_small = "../img/marques/marque_small".$valeur.".jpg";
					}
				if($type==IMAGETYPE_PNG) {
					$path = "../img/marques/marque".$valeur.".png";
					$path_sql = "img/marques/marque".$valeur.".png";
					$path_small = "../img/marques/marque_small".$valeur.".png";
					}
				if($type==IMAGETYPE_GIF) {
					$path = "../img/marques/marque".$valeur.".gif";
					$path_sql = "img/marques/marque".$valeur.".gif";
					$path_small = "../img/marques/marque_small".$valeur.".gif";
					}
				//Copie le fichier dans le répertoire de destination
				if(move_uploaded_file($_FILES['marque']['tmp_name'], $path)){//Le fichier a été uploadé correctement
					 ...
					 // Lit les dimensions de l'image
					   ...
					   ImageDestroy($src_im);
						...
						$query = "SELECT `image` FROM marque_vehicule WHERE `id`=" . $id ;
						$result = mysql_query($query);
						$line = mysql_fetch_row($result);
						if(mysql_num_rows($result)){
							$chemin_image = $line[0];
							//ne pas supprimer l image si elle est celle du system par defaut
							if (strcmp($chemin_image,"img/marques/marquedefault.png")!=0)	{
								unlink("../".$chemin_image);
								unlink("../".str_replace("s/marque","s/marque_small",$chemin_image));
							}
						}
	...
	if($erreur == 1)
	{
		...
		$header = "location:../marques.php?action=modifier&&id_marque=".$id;
	...
	$requete = "UPDATE `marque_vehicule` SET 
	...
	$requete = $requete." WHERE `id`=".$id;
        ...
        if(mysql_query($requete)){
			...
			$header = "location:../marques.php?action=detailler&&id_marque=".$id."&&message_success=L'enregistrement a bien été modifié.";
			header($header);
			exit;
		}
		else
...
function insert() {
	...
	if(!empty($_FILES['marque']['tmp_name']) AND is_uploaded_file($_FILES['marque']['tmp_name'])){
		//echo "dans traitement de fichier\n";
		//On va vérifier la taille du fichier en ne passant pas par $_FILES['avatar']['size'] pour éviter les failles de sécurité
		if(filesize($_FILES['marque']['tmp_name'])<1200000){
			//On vérifie maintenant le type de l'image à l'aide de la fonction getimagesize()
			list($largeur, $hauteur, $type, $attr)=getimagesize($_FILES['marque']['tmp_name']);
			//Si le Type est JPEG (correspond au chiffre 2) on copie l'image
			if(($type==IMAGETYPE_JPEG)||($type==IMAGETYPE_PNG)||($type==IMAGETYPE_GIF)){
	...
	if($erreur == 1)
	{
		...
		$header = "location:../marques.php?action=ajouter";
		$header = $header."&&nom=".$nom;
		if(strcmp($erreur_nom,"")!=0) {
			$header = $header."&&erreur_nom=".$erreur_nom;
		...
		exit;
          ...
          //requete
	$requete = "INSERT into `marque_vehicule` (`nom`) VALUES( 
	'".mysql_real_escape_string($nom)."')" ;
	
	
	if (strcmp($path,"")!=0){
		$requete = "INSERT into `marque_vehicule` (`nom`,`image`) VALUES( 
		'".mysql_real_escape_string($nom)."',
		'".$path_sql."')" ;
       ...
       if(mysql_query($requete)){
       		...
       		$last_id = $ligne[0];
			$header = "location:../marques.php?action=detailler&&id_marque=".$last_id."&&message_success=L'enregistrement a bien été inséré.";
...
function delete() {
	...
	if(isset($_POST['id'])){
		$id=$_POST['id'];
			...
			$query = "delete  FROM marque_vehicule WHERE `id`=" . $id ;
