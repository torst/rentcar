...
<?php
      ...
      switch ( $option ) {
		  case 'update':
		  	if(isset($_SESSION["modifier_villes"]))
		    	update();
		    break;
		    
		  case 'insert':
		  	if(isset($_SESSION["ajouter_villes"]))
		    	insert();
		    break;
		
		  case 'delete':
		  	if(isset($_SESSION["supprimer_villes"]))
		    	delete();
		    break;
...
if(isset($_POST['id_villes'])){
	...
	//unicite login
	include("../config.php");
	$query = "SELECT `nom` FROM ville WHERE `nom`='" . mysql_real_escape_string($nom) . "' and `id`<> ".$id;
	...
	if($erreur == 1)
	{
		...
		$header = "location:../villes.php?action=modifier&&id_villes=".$id;
	...
	$requete = $requete." WHERE `id`=".$id;
        ...
        if(mysql_query($requete)){
			$header = "location:../villes.php?action=detailler&&id_villes=".$id."&&message_success=L'enregistrement a bien été modifié.";
			header($header);
			exit;
		}
		else
...
function insert() {
	...
	//unicite login
	include("../config.php");
	$query = "SELECT `nom` FROM ville WHERE `nom`='" . mysql_real_escape_string($nom) . "' ";
	$result = mysql_query($query);
	if(mysql_num_rows($result)){
    	$erreur_nom = "Le Nom renseigné est déjà attribué, veuillez renseigner un Nom correct";
		$erreur=1;
	...
	if($erreur == 1)
	{
		...
		$header = "location:../villes.php?action=ajouter";
		$header = $header."&&nom=".$nom;
		$header = $header."&&erreur=".$erreur;
		if(strcmp($erreur_nom,"")!=0) {
			$header = $header."&&erreur_nom=".$erreur_nom;
		...
		exit;
          ...
          //requete
	$requete = "INSERT into `ville` (`nom`,`pays`) VALUES( 
	'".mysql_real_escape_string($nom)."',".mysql_real_escape_string($pays).")" ;
       ...
       if(mysql_query($requete)){
       		...
       		$last_id = $ligne[0];
			$header = "location:../villes.php?action=detailler&&id_villes=".$last_id."&&message_success=L'enregistrement a bien été inséré.";
