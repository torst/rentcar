...
<?php
      ...
      switch ( $option ) {
		  ...
		  case 'insert':
		  	if(isset($_SESSION["ajouter_categorie"]))
		    	insert();
		    break;
		
		  case 'delete':
		  	if(isset($_SESSION["supprimer_categorie"]))
		    	delete();
		    break;
...
if(isset($_POST['id_categorie'])){
	...
	$erreur = 0;
	if(strcmp($nom,"")==0) {
		$erreur_nom = "Veuillez renseigner le nom de la catégorie";
		$erreur=1;
	}
	if(strcmp($description,"")==0) {
		$erreur_description = "Veuillez renseigner la description de la catégorie";
		$erreur=1;
	}
	...
	if(!empty($_FILES['categorie']['tmp_name']) AND is_uploaded_file($_FILES['categorie']['tmp_name'])){
		...
		if(filesize($_FILES['categorie']['tmp_name'])<12000000){
			//On vérifie maintenant le type de l'image à l'aide de la fonction getimagesize()
			list($largeur, $hauteur, $type, $attr)=getimagesize($_FILES['categorie']['tmp_name']);
			...
			if(($type==IMAGETYPE_JPEG)||($type==IMAGETYPE_PNG)||($type==IMAGETYPE_GIF)){
				...
				if($type==IMAGETYPE_JPEG) {
					$path = "../img/categories/categorie".$valeur.".jpg";
					$path_sql = "img/categories/categorie".$valeur.".jpg";
					$path_small = "../img/categories/categorie_small".$valeur.".jpg";
					}
				if($type==IMAGETYPE_PNG) {
					$path = "../img/categories/categorie".$valeur.".png";
					$path_sql = "img/categories/categorie".$valeur.".png";
					$path_small = "../img/categories/categorie_small".$valeur.".png";
					}
				if($type==IMAGETYPE_GIF) {
					$path = "../img/categories/categorie".$valeur.".gif";
					$path_sql = "img/categories/categorie".$valeur.".gif";
					$path_small = "../img/categories/categorie_small".$valeur.".gif";
					}
				//Copie le fichier dans le répertoire de destination
				if(move_uploaded_file($_FILES['categorie']['tmp_name'], $path)){//Le fichier a été uploadé correctement
					 ...
					 // Lit les dimensions de l'image
					   ...
					   ImageDestroy($src_im);
						...
						$query = "SELECT `image` FROM categorie WHERE `id`=" . $id ;
						$result = mysql_query($query);
						$line = mysql_fetch_row($result);
						if(mysql_num_rows($result)){
							$chemin_image = $line[0];
							//ne pas supprimer l image si elle est celle du system par defaut
							if (strcmp($chemin_image,"img/categories/categoriedefault.png")!=0)	{
								unlink("../".$chemin_image);
								unlink("../".str_replace("s/categorie","s/categorie_small",$chemin_image));
							}
	...
	if($erreur == 1)
	{
		...
		$header = "location:../categories.php?action=modifier&&id_categorie=".$id;
	...
	$requete = $requete." WHERE `id`=".$id;
        ...
        if(mysql_query($requete)){
			...
			$header = "location:../categories.php?action=detailler&&id_categorie=".$id."&&message_success=L'enregistrement a bien été modifié.";
			header($header);
			exit;
		}
		else
...
function insert() {
	...
	if(!empty($_FILES['categorie']['tmp_name']) AND is_uploaded_file($_FILES['categorie']['tmp_name'])){
		//echo "dans traitement de fichier\n";
		//On va vérifier la taille du fichier en ne passant pas par $_FILES['avatar']['size'] pour éviter les failles de sécurité
		if(filesize($_FILES['categorie']['tmp_name'])<1200000){
			//On vérifie maintenant le type de l'image à l'aide de la fonction getimagesize()
			list($largeur, $hauteur, $type, $attr)=getimagesize($_FILES['categorie']['tmp_name']);
			//Si le Type est JPEG (correspond au chiffre 2) on copie l'image
			if(($type==IMAGETYPE_JPEG)||($type==IMAGETYPE_PNG)||($type==IMAGETYPE_GIF)){
	...
	if($erreur == 1)
	{
		...
		$header = "location:../categories.php?action=ajouter";
		$header = $header."&&nom=".$nom;
		$header = $header."&&description=".urlencode($description);
		if(strcmp($erreur_nom,"")!=0) {
			$header = $header."&&erreur_nom=".$erreur_nom;
	...
	if (strcmp($path,"")!=0){
		$requete = "INSERT into `categorie` (`nom`,`description`,`image`) VALUES( 
		'".mysql_real_escape_string($nom)."',
		'".mysql_real_escape_string($description)."',
		'".$path_sql."')" ;
       ...
       if(mysql_query($requete)){
       		...
       		$last_id = $ligne[0];
			$header = "location:../categories.php?action=detailler&&id_categorie=".$last_id."&&message_success=L'enregistrement a bien été inséré.";
