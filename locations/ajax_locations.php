...
//recuperation date debut location
...
if(strcmp($id_vehicule,"")==0) {
	echo "Veuillez renseigner un vehicule"; ////////////////////////////////erreur traitement
	exit;
}
//calcule prix modele vehicule
$query = "SELECT 
	prix_court_duree, prix_longue_duree, categorie , id
	from prix_modele, modele_vehicule 
	WHERE 
	modele_vehicule.`id`= (select modele from vehicule where id = ?1)
	and prix_modele.`modele` = modele_vehicule.`id`
	";
$query = str_replace("?1",mysql_real_escape_string($id_vehicule),$query);
...
//echo $query;
	...
	while($ligne=mysql_fetch_row($result)){
		...
		$periode_coeff = $ligne[2];
		$prix_promotion = "";
		//echo "prix promotion hors if ".$prix_promotion." \n";
		//mettre le prix promotionnel si existe. Priorité a la promo sur modele
		if(($ligne[5]<>"") OR ($ligne[6]<>"")){
			if($nombre_jour < $seuil){
				$prix_promotion = $ligne[5]; //si vide, prix standrd
