...
<section id="main-content">
          <section class="wrapper">
              ...
              <section class="panel" id="advanced_search">
                  ...
                  <div class="panel-body">
                  <div>
                          	<form class="form-horizontal" role="form" name="recherche_avancee" id="recherche_avancee" method="POST" action="locations.php">
                                  ...
                                  <div class="form-group">
                                      <label  class="col-lg-2 control-label">Statut location </label>
                                      <div class="col-lg-3">
                                      ...
                                      <div class="col-lg-3">
                                          <select class="form-control" name="statut_location" id="statut_location"> <option value=""> </option>
                                          <?php 
                                            $query_statut_location = "SELECT 
                                              `statut_location`.`id`,
                                              `statut_location`.`libelle` 
                                              FROM 
                                              statut_location order by id
                                              ";
                              
                                            $result_statut_location = mysql_query($query_statut_location);
                                                                while($ligne_statut_location=mysql_fetch_row($result_statut_location)){

                                                              ?>
                                          <option value="<?php echo $ligne_statut_location[0]; ?>"><?php echo $ligne_statut_location[1]; ?></option>
                                  ...
                                  <div class="form-group">
                                      <label  class="col-lg-2 control-label">Véhicule</label>
                                      <div class="col-lg-3">
                                          <select  value="" class="form-control" id="operateur_vehicule" name = "operateur_vehicule" placeholder=" " >
                                            <option value="99"></option>
                                            <option value="4">Egale à</option>
                                          </select>
                                      </div>
                                      <div class="col-lg-3">
                                          <select name ="vehicule" id ="vehicule" placeholder="Sélectionner un vehicule..."></select>
                                      </div>
                                  </div>
                      ...
                      <div class="adv-table editable-table ">
                          ...
                          <table class="table table-striped table-hover table-bordered" id="editable-sample">
                              ...
                              $query = "SELECT 
                                ...
                                LEFT JOIN statut_location as sr ON r.statut_location = sr.id
                              ...
                              if(isset($_POST["advanced_query"])){
                               //preparation requete
                                ...
                                $query = $query.custom_request("statut_location", "NOMBRE", $_POST["statut_location"], $_POST["operateur_statut"]);
                                ...
                                $query = $query.custom_request("vehicule", "NOMBRE", $_POST["vehicule"], $_POST["operateur_vehicule"]);


                                echo "Résultat de votre recherche. <a href='locations.php'>Cliquez ici pour TOUT afficher</a>";
							...
							while($ligne=mysql_fetch_row($result)){
                              ...
                              <tr class="" id="<?php echo $ligne[0]; ?>">
                                  ...
                                  <td>
                                  <?php
										if(isset($_SESSION["zoom_locations"])){ 
									?>
                                  	<a id="detail" class=" btn btn-primary  btn-xs" href="locations.php?action=detailler&&id_location=<?php echo $ligne[0]; ?>">
									...
									?>
                                  	<a id="remove-all" class=" btn btn-warning  btn-xs" href="locations.php?action=modifier&&id_location=<?php echo $ligne[0]; ?>"><i class=" icon-edit"></i></a>
                                    <?php
										}
									?>
                                    <?php
										if(isset($_SESSION["supprimer_locations"])){ 
									?>
...
<script type="text/javascript" src="assets/jquery-multi-select/js/jquery.multi-select.js"></script>
    ...
    <script src="js/locations/advanced-multiselect.js"></script>
    ...
    <script >
          ...
          $('#modele').selectize({
            ...
            valueField: 'id',
            labelField: 'marque',
            searchField: ['marque', 'modele', 'matricule'],
            options: [
            <?php
              $requete_modele = "SELECT 
                                  vehicule.id, marque_vehicule.nom, modele_vehicule.nom , matricule

                                  FROM `modele_vehicule`, marque_vehicule , vehicule

                                  WHERE 
                                  modele_vehicule.marque = marque_vehicule.id 
                                  and modele_vehicule.id = vehicule.modele

                                  order by marque_vehicule.nom";
              $result_modele = mysql_query($requete_modele);
              while($ligne_modele=mysql_fetch_row($result_modele)){
              ?>
                {modele: '<?php echo $ligne_modele[2]; ?>', marque: '<?php echo $ligne_modele[1]; ?>', matricule: '<?php echo $ligne_modele[3]; ?>', id: '<?php echo $ligne_modele[0]; ?>'},
            ...
            render: {
              item: function(item, escape) {
                return '<div>' +
                  ...
                  (item.modele ? '<span class="zbanzai-matricule-selectize"> ' + escape(item.matricule) + '</span>' : '') +
                '</div>';
              },
              option: function(item, escape) {
                ...
                //var caption = (item.marque) ? (item.modele) : null; 
                var matricule = ' V: '+item.matricule;
                return '<div>' +
                  
                  (caption ? '<span class="caption"> ' + escape(caption) + '</span>' : '') +
                  '<span class="zbanzai-matricule-selectize">' + escape(matricule) + '</span>' +
                '</div>';
