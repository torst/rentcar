...
<?php
      ...
      switch ( $option ) {
		  ...
		  case 'insert':
		  	if(isset($_SESSION["ajouter_categorie_promotions"]))
		    	insert();
		    break;
		
		  case 'delete':
		  	if(isset($_SESSION["supprimer_categorie_promotions"]))
		    	delete();
		    break;
...
function update() {
if(isset($_POST['id_categorie_promotions'])){
			
	$id=$_POST['id_categorie_promotions'];
	...
	//unicite categorie
	include("../config.php");
	$query = "SELECT `categorie` FROM promotion_categorie WHERE `categorie`='" . mysql_real_escape_string($categorie) . "' and `categorie`<> ".$id;
	$result = mysql_query($query);
	if(mysql_num_rows($result)){
    	$erreur_categorie = "Le modèle renseigné est déjà en promotion, veuillez renseigner un autre modèle";
		$erreur=1;
	}
	...
	if($erreur == 1)
	{
		...
		$header = "location:../categorie_promotions.php?action=modifier&&id_categorie_promotions=".$id;
		...
		exit;
          ...
          //requete
	$requete = "UPDATE `promotion_categorie` SET 
	...
	$requete = $requete." WHERE `categorie`=".$id;
	
        if(mysql_query($requete)){
			$header = "location:../categorie_promotions.php?action=detailler&&id_categorie_promotions=".$categorie."&&message_success=L'enregistrement a bien été modifié.";
			header($header);
			exit;
		}
		else
...
function insert() {
	...
	//unicite categorie
	include("../config.php");
	$query = "SELECT `categorie` FROM promotion_categorie WHERE `categorie`='" . mysql_real_escape_string($categorie) . "' ";
	$result = mysql_query($query);
	if(mysql_num_rows($result)){
    	$erreur_categorie = "Le modèle renseigné est déjà en promotion, veuillez renseigner un autre modèle";
		$erreur=1;
	...
	if($erreur == 1)
	{
		...
		$header = "location:../categorie_promotions.php?action=ajouter";
		$header = $header."&&categorie=".$categorie;
		$header = $header."&&erreur=".$erreur;
		$header = $header."&&date_debut=".$date_debut;
		$header = $header."&&date_fin=".$date_fin;
		...
		exit;
          ...
          //requete
	$requete = "INSERT into `promotion_categorie` (`categorie`,`prix_court_duree`,`prix_longue_duree`,`date_debut`,`date_fin`) VALUES( 
	...
	STR_TO_DATE('".mysql_real_escape_string($date_fin)."', '%d/%m/%Y'))" ;
       ...
       if(mysql_query($requete)){
			...
			$header = "location:../categorie_promotions.php?action=detailler&&id_categorie_promotions=".$categorie."&&message_success=L'enregistrement a bien été inséré.";
...
function delete() {
	...
	if(isset($_POST['id'])){
		$id=$_POST['id'];
			...
			$query = "delete  FROM promotion_categorie WHERE `categorie`=" . $id ;
