...
<?php
      ...
      switch ( $option ) {
		  ...
		  case 'insert':
		  	if(isset($_SESSION["ajouter_service_modele_promotions"]))
		    	insert();
		    break;
		
		  case 'delete':
		  	if(isset($_SESSION["supprimer_service_modele_promotions"]))
		    	delete();
		    break;
...
function update() {
if(isset($_POST['modele']) && isset($_POST['service'])){
	...
	$trouve=0;
	$query = "SELECT `modele`,`service` FROM promotion_service_modele WHERE `modele`=" . mysql_real_escape_string($modele) . " and `service`=" . mysql_real_escape_string($service) ;
	$result = mysql_query($query);
	while($ligne=mysql_fetch_row($result)){
	 $trouve=1;
	 if( $ligne[0] == $id_modele && $ligne[1] == $id_service)
	  $trouve=0;
	}
	if($trouve==1){
    	$erreur_modele = "Le service renseigné est déjà en promotion pour ce modèle, veuillez renseigner un autre couple (service,modèle)";
		$erreur=1;
	}
	...
	if($erreur == 1)
	{
		...
		$header = "location:../service_modele_promotions.php?action=modifier&&id_service=".$id_service."&&id_modele=".$id_modele;
		...
		exit;
          ...
          //requete
	$requete = "UPDATE `promotion_service_modele` SET 
	...
	$requete = $requete." and `service`=".$id_service;
        ...
        if(mysql_query($requete)){
			$header = "location:../service_modele_promotions.php?action=detailler&&id_service=".$service."&&id_modele=".$modele."&&message_success=L'enregistrement a bien été modifié.";
			header($header);
			exit;
		}
		else
...
function insert() {
	...
	//unicite modele
	include("../config.php");
	$query = "SELECT `modele` FROM promotion_service_modele WHERE `modele`='" . mysql_real_escape_string($modele) . "' and `service`='" . mysql_real_escape_string($service) . "' ";
	$result = mysql_query($query);
	if(mysql_num_rows($result)){
    	$erreur_modele = "Le service renseigné est déjà en promotion pour ce modèle, veuillez renseigner un autre service ou un autre modèle";
		$erreur=1;
	}
	//test si prix est numerique
	if(!is_numeric($prix_court)) {
	...
	if($erreur == 1)
	{
		...
		$header = "location:../service_modele_promotions.php?action=ajouter";
		$header = $header."&&modele=".$modele;
		$header = $header."&&erreur=".$erreur;
		$header = $header."&&date_debut=".$date_debut;
		$header = $header."&&date_fin=".$date_fin;
		...
		exit;
          ...
          //requete
	$requete = "INSERT into `promotion_service_modele` (`modele`,`service`,`prix_court_duree`,`prix_longue_duree`,`date_debut`,`date_fin`) VALUES( 
	...
	STR_TO_DATE('".mysql_real_escape_string($date_fin)."', '%d/%m/%Y'))" ;
       ...
       if(mysql_query($requete)){
			...
			$header = "location:../service_modele_promotions.php?action=detailler&&id_modele=".$modele."&&id_service=".$service."&&message_success=L'enregistrement a bien été inséré.";
...
function delete() {
	...
	if(isset($_POST['id_service']) && isset($_POST['id_modele'])){
		$id_service=$_POST['id_service'];
		$id_modele=$_POST['id_modele'];
					
			include("../config.php");
			
			$query = "delete  FROM promotion_service_modele WHERE `modele`=" . $id_modele." and `service`=" . $id_service ;
