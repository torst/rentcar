...
<?php
      ...
      switch ( $option ) {
		  ...
		  case 'insert':
		  	if(isset($_SESSION["ajouter_service"]))
		    	insert();
		    break;
		
		  case 'delete':
		  	if(isset($_SESSION["supprimer_service"]))
		    	delete();
		    break;
...
if(isset($_POST['id_service'])){
	...
	$erreur = 0;
	if(strcmp($nom,"")==0) {
		$erreur_nom = "Veuillez renseigner le nom du service";
		$erreur=1;
	}
	if(strcmp($description,"")==0) {
		$erreur_description = "Veuillez renseigner la description du service";
		$erreur=1;
	}
	if(strcmp($prix,"")==0) {
		$erreur_prix = "Veuillez renseigner le prix du service";
		$erreur=1;
	}
	...
	if(!empty($_FILES['service']['tmp_name']) AND is_uploaded_file($_FILES['service']['tmp_name'])){
		...
		if(filesize($_FILES['service']['tmp_name'])<12000000){
			//On vérifie maintenant le type de l'image à l'aide de la fonction getimagesize()
			list($largeur, $hauteur, $type, $attr)=getimagesize($_FILES['service']['tmp_name']);
			...
			if(($type==IMAGETYPE_JPEG)||($type==IMAGETYPE_PNG)||($type==IMAGETYPE_GIF)){
				...
				if($type==IMAGETYPE_JPEG) {
					$path = "../img/services/service".$valeur.".jpg";
					$path_sql = "img/services/service".$valeur.".jpg";
					$path_small = "../img/services/service_small".$valeur.".jpg";
					}
				if($type==IMAGETYPE_PNG) {
					$path = "../img/services/service".$valeur.".png";
					$path_sql = "img/services/service".$valeur.".png";
					$path_small = "../img/services/service_small".$valeur.".png";
					}
				if($type==IMAGETYPE_GIF) {
					$path = "../img/services/service".$valeur.".gif";
					$path_sql = "img/services/service".$valeur.".gif";
					$path_small = "../img/services/service_small".$valeur.".gif";
					}
				//Copie le fichier dans le répertoire de destination
				if(move_uploaded_file($_FILES['service']['tmp_name'], $path)){//Le fichier a été uploadé correctement
					 ...
					 // Lit les dimensions de l'image
					   ...
					   ImageDestroy($src_im);
						...
						$query = "SELECT `image` FROM service WHERE `id`=" . $id ;
						$result = mysql_query($query);
						$line = mysql_fetch_row($result);
						if(mysql_num_rows($result)){
							$chemin_image = $line[0];
							//ne pas supprimer l image si elle est celle du system par defaut
							if (strcmp($chemin_image,"img/services/servicedefault.png")!=0)	{
								unlink("../".$chemin_image);
								unlink("../".str_replace("s/service","s/service_small",$chemin_image));
							}
						}
	...
	if($erreur == 1)
	{
		...
		$header = "location:../services.php?action=modifier&&id_service=".$id;
	...
	$requete = $requete." WHERE `id`=".$id;
        ...
        if(mysql_query($requete)){
			...
			$header = "location:../services.php?action=detailler&&id_service=".$id."&&message_success=L'enregistrement a bien été modifié.";
			header($header);
			exit;
		}
		else
...
function insert() {
	...
	if(!empty($_FILES['service']['tmp_name']) AND is_uploaded_file($_FILES['service']['tmp_name'])){
		//echo "dans traitement de fichier\n";
		//On va vérifier la taille du fichier en ne passant pas par $_FILES['avatar']['size'] pour éviter les failles de sécurité
		if(filesize($_FILES['service']['tmp_name'])<1200000){
			//On vérifie maintenant le type de l'image à l'aide de la fonction getimagesize()
			list($largeur, $hauteur, $type, $attr)=getimagesize($_FILES['service']['tmp_name']);
			//Si le Type est JPEG (correspond au chiffre 2) on copie l'image
			if(($type==IMAGETYPE_JPEG)||($type==IMAGETYPE_PNG)||($type==IMAGETYPE_GIF)){
	...
	if($erreur == 1)
	{
		...
		$header = "location:../services.php?action=ajouter";
		$header = $header."&&nom=".$nom;
		$header = $header."&&description=".urlencode($description);
		if(strcmp($erreur_nom,"")!=0) {
			$header = $header."&&erreur_nom=".$erreur_nom;
		...
		exit;
          ...
          //requete
	$requete = "INSERT into `service` (`nom`,`description`,`prix`,`periode`) VALUES( 
	...
	if (strcmp($path,"")!=0){
		$requete = "INSERT into `service` (`nom`,`description`,`prix`,`periode`,`image`) VALUES( 
		'".mysql_real_escape_string($nom)."',
		'".mysql_real_escape_string($description)."',
		".mysql_real_escape_string($prix).",
		".mysql_real_escape_string($periode).",
       ...
       if(mysql_query($requete)){
       		...
       		$last_id = $ligne[0];
			$header = "location:../services.php?action=detailler&&id_service=".$last_id."&&message_success=L'enregistrement a bien été inséré.";
